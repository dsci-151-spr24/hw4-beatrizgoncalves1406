...
gss16 <- gss16 %>%
  mutate(polviews = factor(polviews, levels = c("Extremely liberal", "Liberal", "Vaguely liberal", "Moderate", "Extremely conservative", "Vaguely conservative", "Conservative"), labels = c("Liberal", "Liberal", "Liberal", "Moderate", "Conservative", "Conservative", "Conservative")))
...
```{r eval=FALSE}
gss16_rec_1 <- recipe(advfront ~ polviews + educ + wrkstat, data = gss16_train) %>%
...
***The accuracy varies between the folds with the values between 0.82 to 0.87. The Brier score varies between 0.10 to 0.15. The ROC curve has a greater variation with the range of 0.54 to 0.75. The average area under the ROC curve across all folds is 0.64, while the average accuracy is 0.84. Overall, the accuracy and brier score are more consistent across the data compared to the ROC curve.***
...
***The accuracies and the brier score of this model has the same variation as the previous model. However, the area of the ROC curve of this model has a slightly broader range, from 0.54 to 0.77. Overall, the average area under the ROC curve across all folds has slightly increased, at 0.65, while the average accuracy is still 0.84.***
...
***The models' performance are slightly even, with the outcome being slightly in favor of the model that does not include 'wrkstat.'***
...
***These models are used to predict the testing data and are both perform worse than on the training data. This suggests that this model may have been over fitting points on the training data instead of fitting the general distribution of the data. The variable `wrkstat` does better at making the prediction more general and less specific. With that being said, the lower value in the training data, the higher value in the testing data.***
...
***There were 237 people that responded "Yes" and 1136 people that responded "No".***
